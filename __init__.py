...
Path.mkdir(dg_path, exist_ok= True)
pythonversion = f"python{sys.version_info.major}{sys.version_info.minor}"
Path.mkdir(dg_path / pythonversion, exist_ok=True)
...
def save_settings(settings_filepath, settings):
    with open(settings_filepath, 'w') as f:
        json.dump(settings, f, indent=2)

DEFAULT_SETTINGS.update(load_settings(settings_filepath))
print(DEFAULT_SETTINGS['device'])

try:
    install_packages.ensure_package_path(DEFAULT_SETTINGS['device'],target = DEFAULT_SETTINGS["dependency_path"])
...
def check_model_avaibility(self, context):
...
class DepthGeniusProperties(bpy.types.PropertyGroup):
    model_size: bpy.props.EnumProperty(
        ...
        items=[
            ...
            # ('vitg', "ViT-G", "Giant model (non-commercial use only)"),
        ],
        default='vits',
        update=check_model_avaibility
    )
    ...
    installation_in_progress: bpy.props.BoolProperty(
        ...
        description="this flag is set when installtion script is running",
        default=False
    )

    current_model_avaibility: bpy.props.BoolProperty(
        name="Model Available?",
        description="current select model is avialable or not",
        default=True
    )
    ...
    mode: bpy.props.EnumProperty(items=[('IMAGE', 'Single Image', 'Depth Image generated from a single image'),
                                        ('MOVIE', 'Video File', 'Generate Depth Video from selected video file'),
                                        ('BATCH_IMG','Batch Images', 'Bathc generate Depth Images from multiple images from single run')],
    ...
    use_colormap : bpy.props.BoolProperty(default=False, name="Use Colormap")

    colormaps=[('HOT','HOT',''),('COOL','COOL',''),('OCEAN','OCEAN',''), ('SUMMER','SUMMER',''), ('SPRING','SPRING',''),
               ('INFERNO','INFERNO',''),('PLASMA','PLASMA',''),('VIRIDIS','VIRIDIS',''),('TWILIGHT','TWILIGHT','')]
    colormap: bpy.props.EnumProperty(items=colormaps,name="Colormap")

    include_alpha: bpy.props.BoolProperty(name="Include Alpha Channel", description="Copies the source image’s alpha into the exported depth map", 
                                          default=False)
...
classes = (
    ...
    # operators.DEPTHGENIUS_OT_ApplyDisplacement,
...
def unregister():
    ...
    settings["dependency_path"] = str(Path(bpy.context.preferences.addons[__package__].preferences.dependencies_path))
    save_settings(settings_filepath,settings)
    install_packages.unregister()
